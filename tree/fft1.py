...
def train(epoch):
    ...
    if epoch % 20 == 0:
        torch.save(model.state_dict(),"checkpoints/mnist/fft_%03d.pt" % epoch)
    ...
    with torch.no_grad():
        for i, (data, c) in enumerate(test_loader):
            ...
            if i == 0:
                ...
                img = img[:4]
                img = img.view(4, 1, 30, 100)
                save_image(img.cpu(),
                         'images/fft_09_' + str(epoch) + '.png', nrow=1)
                save_image(img1.cpu(),
                         'images/z_09_' + str(epoch) + '.png', nrow=64)
