...
model = VAE()
model.load_state_dict(torch.load('/data/tree/voice/csp_wav_140.pt'))
...
def gen():
    ...
    with torch.no_grad():
        ...
        f0_4 = (f0/400.0).astype(np.float32)
        f0_4 = f0_4.reshape((1, 1, L))
        f0_8 = (f0_8_org/400.0).astype(np.float32)
        f0_8 = f0_8.reshape((1, 1, L))

        data = torch.from_numpy(csp)
        c_4  = torch.from_numpy(c_4)
        c_8  = torch.from_numpy(c_8)
        f0_4 = torch.from_numpy(f0_4)
        f0_8 = torch.from_numpy(f0_8)
        ...
        z_4 = torch.cat((f0_4, c_4), dim=1)
        z_8 = torch.cat((f0_8, c_8), dim=1)
        ...
        img = img.unsqueeze(1)
        save_image(img.cpu(),
                         'images/generate_4_A_240.png', nrow=1)
        ...
        librosa.output.write_wav('images/4_A_240_org.wav', y, sr=fs)
        librosa.output.write_wav('images/4_A_240_rx_4.wav', y4, sr=fs)
        librosa.output.write_wav('images/4_A_240_rx_8.wav', y8, sr=fs)
